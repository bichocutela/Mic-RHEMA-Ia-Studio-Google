...
- A função `pwa-push-send` foi validada novamente com uma sessão administrativa real da PWA: retornou HTTP `200`, reconheceu o administrador e reportou `0` destinatários, sem enviar aviso porque nenhum navegador de teste concedeu a permissão.
- A URL pública normal, sem parâmetro de versão, foi reaberta após o deploy e carregou a PWA atual com o botão “Ativar notificações”, a sessão administrativa e o cache `mic-rhema-pwa-v3`.
