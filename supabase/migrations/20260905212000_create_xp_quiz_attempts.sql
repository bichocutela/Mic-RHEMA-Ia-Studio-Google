create table if not exists public.xp_quiz_attempts (
...
create index if not exists xp_quiz_attempts_member_created_idx
  on public.xp_quiz_attempts(member_id, created_at desc);

alter table public.xp_quiz_attempts enable row level security;
revoke all on public.xp_quiz_attempts from anon, authenticated;
grant select, insert on public.xp_quiz_attempts to service_role;
...
revoke all on function public.xp_submit_quiz(text, text, integer, text, boolean, integer, text) from public, anon, authenticated;
grant execute on function public.xp_submit_quiz(text, text, integer, text, boolean, integer, text) to service_role;
