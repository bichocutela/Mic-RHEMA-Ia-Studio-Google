...
function fromValue(value?: FirestoreValue): unknown {
  ...
  if (value.mapValue !== undefined) return Object.fromEntries(Object.entries(value.mapValue.fields ?? {}).map(([k, x]) => [k, fromValue(x)]));
  return null;
}
function docData(document?: FirestoreDocument | null) {
...
function activityMap(value: unknown): Record<string, string[]> {
  ...
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([k, x]) => [k, stringList(x)]));
}
...
async function resolveMember(
  ...
  const configured = Deno.env.get("SUPABASE_ANON_KEY") ?? "";
  let memberId = "";
  let phone = "";
  let firebase = false;
  const legacy = Boolean(configured) && provided === configured && bearer === configured;
  const publishable = provided === CURRENT_PUBLISHABLE_KEY && bearer === CURRENT_PUBLISHABLE_KEY;
  if (legacy || publishable) {
    ...
    memberId = clean(verified.payload.sub, 200);
    firebase = true;
  ...
  if (firebase && access.isApproved !== true && access.isAdmin !== true) throw new Error("Acesso do membro ainda não aprovado.");
  if (!firebase && normalizePhone(access.phone) !== phone) throw new Error("Identidade do membro não confere.");
  ...
  return {
    ...
    legacyXp: entries.reduce((sum, entry) => sum + parseLegacyXp(entry), 0),
    legacyQuizIds: parseLegacyQuizIds(entries),
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (["hint", "answer", "claim_mission"].includes(action) && !member.firebaseAuth) {
      return json({ error: "Atualize o MIC Rhema e entre novamente para usar o Quiz com segurança." }, 401);
    }
    ...
    const { data: ensured, error: ensureError } = await sb.rpc("xp_ensure_account", {
    ...
    if (member.legacyQuizIds.length) {
      ...
      const { error } = await sb.from("xp_legacy_quiz_receipts").upsert(rows, {
    ...
    // O ponto zero da Jornada precisa existir antes da primeira pergunta pós-Nível 8.
    // Assim os acertos feitos antes do desbloqueio não são confundidos com progresso novo.
    if (member.firebaseAuth && member.xpUnlocked) {
      const { error } = await sb.rpc("xp_capture_journey_baseline", { p_member_id: member.memberId });
      if (error) throw error;
    }
    ...
    if (action === "status" || action === "next_question") {
      ...
      const [attempts, legacy] = await Promise.all([
        sb.from("xp_quiz_attempts").select("question_id").eq("member_id", member.memberId).in("question_id", ids),
        sb.from("xp_legacy_quiz_receipts").select("question_id").eq("member_id", member.memberId).in("question_id", ids),
      ]);
      if (attempts.error) throw attempts.error;
      if (legacy.error) throw legacy.error;
      const answered = new Set([...(attempts.data ?? []), ...(legacy.data ?? [])].map((item: any) => String(item.question_id)));
    ...
    if (action === "hint") {
      ...
      const rank = requested === "easy" || requested === "easy_hint" ? 2
        : requested === "subtle" || requested === "subtle_hint" ? 1 : 0;
      if (!rank) return json({ error: "Dica inválida." }, 400);
      const { data, error } = await sb.rpc("xp_record_quiz_hint", {
        p_member_id: member.memberId,
      ...
      return json({
        ...
        variant,
        hint: effective >= 2 ? question.easyHint : question.hardHint,
        multiplier: effective >= 2 ? .7 : .9,
      });
    }
    ...
    if (action === "answer") {
      ...
      const { data: attempt } = await sb.from("xp_quiz_attempts")
        .select("selected_option,variant,correct")
        .eq("member_id", member.memberId).eq("question_id", question.id).single();
      return json({
        ok: true,
        catalogVersion: QUIZ_CATALOG_VERSION,
        unlocked: member.xpUnlocked,
    ...
    if (action === "claim_mission") {
      ...
      const activity: string | null = missionId.startsWith("easy_") ? "journey_mission_easy"
        : missionId.startsWith("medium_") ? "journey_mission_medium"
        : missionId.startsWith("hard_") ? "journey_mission_hard" : null;
      const amount = activity === "journey_mission_easy" ? 15
        : activity === "journey_mission_medium" ? 35
        : activity === "journey_mission_hard" ? 70 : 0;
