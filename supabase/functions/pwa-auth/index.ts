...
async function findMember(account: ServiceAccount, phone: string): Promise<Member | null> {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    let member: Member;
    if (isAdministrator) {
      member = { id: "admin", name: "Administrador", isApproved: true, isIbr: true, isAdmin: true };
    } else {
      ...
      if (member.name.toLocaleLowerCase("pt-BR") !== name.toLocaleLowerCase("pt-BR")) {
        return json({ error: "Nome ou telefone não correspondem ao cadastro aprovado." }, 403);
      }
    }
