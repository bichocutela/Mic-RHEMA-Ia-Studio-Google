...
import { createRemoteJWKSet, errors as joseErrors, jwtVerify, importPKCS8, SignJWT } from "npm:jose@5.10.0";
...
function normalizePhone(value:unknown){const digits=String(value??"").replace(/\D/g,"");return digits.length>=12&&digits.length<=13&&digits.startsWith("55")?digits.slice(2):digits}
...
Deno.serve(async(request:Request)=>{
  ...
  try{
    ...
    if(typeof data.phone==="string"&&normalizePhone(data.phone)!==normalizePhone(current.phone))throw new HttpError(409,"O telefone é a identidade fixa desta conta e não pode ser alterado. Altere apenas o nome e os demais dados do perfil.");
    ...
    if(allowed.name!==undefined&&!allowed.name)throw new HttpError(400,"O nome não pode ficar vazio.");if(allowed.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(allowed.email))throw new HttpError(400,"Informe um e-mail válido.");
