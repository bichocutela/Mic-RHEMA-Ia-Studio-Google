...
function CourseCover({course,locked}:{course:Course;locked:boolean}){
  ...
  const cover=resolveDisplayImageUrl(course.imageUrl);
  if(!cover||failed)return <span className="ibr-course-cover-fallback" aria-hidden="true">{locked?<LockKeyhole size={19}/>:<School size={20}/>}</span>;
  return <img className="ibr-course-cover" src={cover} alt="" loading="lazy" onError={()=>setFailed(true)}/>;
}
...
export function IbrParityView({ session, onLogin }: { session: PwaSessionLike; onLogin: () => void }) {
  ...
  return <section className="parity-page"><div className="parity-title"><div><p>INSTITUTO BÍBLICO RHEMA</p><h1>Plataforma de Ensino IBR</h1><span>Mesmo progresso, cursos e materiais do Android.</span></div><School size={30}/></div>
    <article className="android-module-card ibr-progress-card">
      <div className="ibr-progress-head"><div className="ibr-progress-copy"><small>PROGRESSO GERAL</small><strong>{percent}% concluído</strong><span>Acompanhe sua evolução na formação bíblica.</span></div><div className="ibr-progress-total"><b>{completedModules}/{ordered.length}</b><small>MÓDULOS</small></div></div>
      <div className="ibr-progress-track" aria-label={`${percent}% concluído`}><i style={{width:`${percent}%`}}/></div>
      <div className="ibr-progress-stats"><div className="ibr-progress-stat"><b>{completedModules}</b><small>Concluído(s)</small></div><div className="ibr-progress-stat"><b>{remainingModules}</b><small>Restante(s)</small></div><div className="ibr-progress-stat"><b>{Math.floor(completedMinutes/60)}h {completedMinutes%60}m</b><small>Tempo estudado</small></div></div>
      {percent>=100&&profile?.ibrCertificateUrl?<><a className="ibr-certificate-action" href={profile.ibrCertificateUrl} target="_blank" rel="noreferrer"><BadgeCheck size={18}/> Ver Certificado</a>{profile.email&&<a className="ibr-certificate-action" href={`mailto:${encodeURIComponent(profile.email)}?subject=${encodeURIComponent("Certificado IBR MIC Rhema")}&body=${encodeURIComponent(`Meu certificado IBR: ${profile.ibrCertificateUrl}`)}`}><Mail size={17}/> Enviar por e-mail</a>}</>:<button className="ibr-certificate-action" disabled><LockKeyhole size={17}/> {percent>=100?"Certificado em processamento":"Certificado bloqueado até concluir os módulos"}</button>}
    </article>
...
function IbrLesson({course,lesson,done,onBack,onComplete}:{course:Course;lesson:Chapter;done:boolean;onBack:()=>void;onComplete:()=>void}){
  ...
  return <section className="parity-page ibr-lesson-view"><button className="back-link" onClick={onBack}><ChevronLeft size={18}/> Voltar às aulas</button><div className="parity-title"><div><p>{course.title||"CURSO IBR"}</p><h1>{lesson.title||"Aula IBR"}</h1><span>{lesson.description||"Acompanhe a aula até o fim para registrar seu progresso."}</span></div></div>
    ...
    {(pdf||docx)&&<section style={{marginTop:22,display:"grid",gap:10}}><p style={{fontWeight:800,margin:0}}>Conteúdos para estudo</p>{pdf&&<><article className="android-module-card" style={{display:"flex",alignItems:"center",gap:12,padding:16}}><FileText size={28}/><div style={{flex:1}}><strong style={{display:"block"}}>Material complementar em PDF</strong><small>Leia dentro da PWA ou baixe para estudar depois.</small></div><button className="back-link" onClick={downloadPdf}><Download size={18}/> Baixar</button></article>{pdfEmbed&&<iframe className="parity-document" style={{width:"100%",minHeight:"62dvh",border:0,borderRadius:16,background:"#fff"}} src={pdfEmbed} title="Material complementar IBR"/>}</>}{docx&&<article className="android-module-card" style={{display:"flex",alignItems:"center",gap:12,padding:16}}><FileText size={28}/><div style={{flex:1}}><strong style={{display:"block"}}>Material complementar em Word (.docx)</strong><small>Abra no Word ou baixe para estudar e editar.</small></div><button className="back-link" onClick={downloadDocx}><Download size={18}/> Baixar</button></article>}</section>}
