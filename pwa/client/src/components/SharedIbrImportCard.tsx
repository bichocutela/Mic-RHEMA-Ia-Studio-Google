...
export function SharedIbrImportCard({ courses }: { courses: AnyDoc[] }) {
  ...
  const attachToLesson = async () => {
    ...
    try {
      ...
      const field = kind === "pdf" ? "studyPdfUrl" : kind === "epub" ? "studyEpubUrl" : "studyDocxUrl";
  ...
  const addToLibrary = async () => {
    ...
    try {
      ...
      await clear();
      toast.success(`${labelForKind(kind)} publicado na biblioteca.`);
    } catch (error) {
  ...
  return <article className="admin-global-card" style={{ display: "grid", gap: 12, marginBottom: 18 }}>
    ...
    <p style={{ margin: 0, opacity: .78 }}>Recebido pelo menu Compartilhar do aparelho. O MIC Rhema identifica PDF, EPUB ou Word e permite anexar direto a uma aula.</p>
    ...
    <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
      <button className="admin-save" disabled={busy || !chapterId || !kind} onClick={() => void attachToLesson()}><FilePlus2 size={18}/>{busy ? "Enviando…" : "Anexar à aula"}</button>
      <button className="admin-secondary" disabled={busy || !kind} onClick={() => void addToLibrary()}><BookOpen size={18}/>Publicar na biblioteca</button>
