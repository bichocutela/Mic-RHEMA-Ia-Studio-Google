...
import { BookOpen, Check, ChevronLeft, Download, Headphones, Image as ImageIcon, Play, Search, SortAsc, Video, X } from "lucide-react";
import { listenToCollection } from "@/lib/firebase";
import { recordPwaActivity } from "@/lib/badge-activity";
import { contentTimestamp, normalizeSearch, safeFilename, youtubeThumbnail, youtubeVideoId } from "@/lib/parity-utils";
...
function mediaUrl(item:ViewItem){if(item.kind==="Fotos")return item.driveFolderUrl||"";return item.videoUrl||item.audioUrl||item.bookUrl||item.mediaUrl||""}
...
function image(item:ViewItem){if(item.kind==="Fotos")return item.coverUrl||item.photos?.[0]?.url||"";if(item.kind==="Vídeo"){const official=youtubeThumbnail(item.videoUrl||item.mediaUrl);if(official)return official;}return item.imageUrl||item.thumbnailUrl||item.coverUrl||""}
...
export function MediaParityViewV2(){
  ...
  return <section className="parity-page media-parity"><div className="parity-title"><div><p>MÍDIA</p><h1>Conteúdo da igreja</h1><span>Livros, áudios, vídeos e fotos sincronizados em tempo real.</span></div><Play size={30}/></div>
    ...
    {!visible.length?<p className="parity-status">Nenhum conteúdo encontrado.</p>:<div className="media-parity-grid">{visible.map(item=><button key={`${item.kind}-${item.id}`} onClick={()=>openItem(item)}><div className="media-cover">{image(item)?<img src={image(item)} alt="" loading="lazy"/>:item.kind==="Áudio"?<Headphones/>:item.kind==="Livro"?<BookOpen/>:item.kind==="Fotos"?<ImageIcon/>:<Video/>}</div><span>{item.kind}</span><strong>{item.title||"Conteúdo MIC Rhema"}</strong><small>{secondary(item)||"Abrir conteúdo"}</small></button>)}</div>}
  </section>
}

function MediaReaderV2({item,onBack}:{item:ViewItem;onBack:()=>void}){
  const audioRef=useRef<HTMLAudioElement|null>(null);const url=mediaUrl(item);const yid=item.kind==="Vídeo"?youtubeVideoId(url):"";
  const download=()=>{if(!url)return;const anchor=document.createElement("a");anchor.href=url;anchor.target="_blank";anchor.rel="noopener noreferrer";if(item.kind==="Livro")anchor.download=`${safeFilename(item.title||"livro-mic-rhema")}.pdf`;anchor.click()};
  if(item.kind==="Fotos"){const photos=(item.photos||[]).filter(photo=>photo.url);return <section className="parity-page"><button className="back-link" onClick={onBack}><ChevronLeft size={18}/> Voltar à mídia</button><div className="parity-title"><div><p>FOTOS</p><h1>{item.title||"Álbum"}</h1><span>{item.description||`${photos.length} foto(s)`}</span></div></div><div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}}>{photos.map((photo,index)=><figure key={`${photo.url}-${index}`} style={{margin:0}}><a href={photo.url} target="_blank" rel="noreferrer"><img src={photo.url} alt={photo.caption||`Foto ${index+1}`} loading="lazy" style={{width:"100%",aspectRatio:"1",objectFit:"cover",borderRadius:16}}/></a>{photo.caption&&<figcaption style={{fontSize:12,marginTop:4}}>{photo.caption}</figcaption>}</figure>)}</div>{item.driveFolderUrl&&<a className="parity-primary" href={item.driveFolderUrl} target="_blank" rel="noreferrer">Abrir álbum completo</a>}</section>}
  return <section className="parity-page media-reader"><button className="back-link" onClick={onBack}><ChevronLeft size={18}/> Voltar à mídia</button><div className="parity-title"><div><p>{item.kind.toUpperCase()}</p><h1>{item.title||"Conteúdo"}</h1><span>{secondary(item)||"MIC Rhema"}</span></div></div>
    {item.kind==="Vídeo"?(yid?<iframe className="parity-video" title={item.title||"Vídeo"} src={`https://www.youtube-nocookie.com/embed/${yid}?rel=0&playsinline=1`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>:<video className="parity-video" controls playsInline src={url}/>):item.kind==="Áudio"?<div className="audio-reader"><Headphones size={42}/><audio ref={audioRef} controls src={url}/><div className="audio-skip"><button onClick={()=>{if(audioRef.current)audioRef.current.currentTime=Math.max(0,audioRef.current.currentTime-15)}}>-15s</button><button onClick={()=>{if(audioRef.current)audioRef.current.currentTime+=15}}>+15s</button><button onClick={()=>{if(audioRef.current)audioRef.current.playbackRate=audioRef.current.playbackRate>=2?0.75:Math.min(2,audioRef.current.playbackRate+0.25)}}>Velocidade</button></div></div>:<><iframe className="parity-document" title={item.title||"PDF"} src={url}/><button className="parity-primary" onClick={download}><Download size={18}/> Baixar PDF</button></>}
