...
export function ProfileParityViewV2({ session, onNavigateHome }: { session: PwaSessionLike; onNavigateHome: () => void }) {
  ...
  const profilePhotoSelected=draft.avatarId.startsWith("profile_photo::");
  const previousAvatarId=profilePhotoSelected?(draft.avatarId.split("::").filter(Boolean).at(-1)||"davi"):draft.avatarId;
  const avatar=avatars.find((item)=>item.id===previousAvatarId)||avatars[0];
  const avatarLabel=profilePhotoSelected?"Minha foto":avatar.name;
  const equipped=badgeForId(draft.equippedBadgeId);
  const persist=async(next:PwaMemberProfile,success:string)=>{setSaving(true);try{const updated=await savePwaMemberProfile({name:next.name,phone:next.phone,address:next.address,birthDate:next.birthDate,email:next.email,avatarId:next.avatarId,equippedBadgeId:next.equippedBadgeId});const merged={...updated,profilePhotoUrl:updated.profilePhotoUrl||next.profilePhotoUrl||profile.profilePhotoUrl||""};setProfile(merged);setDraft(merged);const stored=localStorage.getItem("micrhema-pwa-session");if(stored){const parsed=JSON.parse(stored);parsed.name=updated.name;localStorage.setItem("micrhema-pwa-session",JSON.stringify(parsed));}toast.success(success);}catch(error){toast.error(error instanceof Error?error.message:"Não foi possível salvar.");setDraft(profile);}finally{setSaving(false)}};
  ...
  return <section className="parity-page profile-v2-root">
    <header className="profile-v2-hero"><PwaProfileLiveShowcase memberId={profile.id} avatarId={draft.avatarId} profilePhotoUrl={draft.profilePhotoUrl||""} badgeId={equipped.id} size={96}/><div><p>{session.isAdmin?"ADMINISTRADOR · ":""}SEU AVATAR BÍBLICO</p><h1>{draft.name}</h1><span>{avatarLabel} · Nível {equipped.level||1}: {equipped.name}</span></div></header>
    ...
    {previewBadge&&<div className="profile-v2-badge-modal" role="presentation" onClick={()=>setPreviewBadgeId(null)}>
      <article role="dialog" aria-modal="true" aria-label={`Emblema ${previewBadge.name}`} onClick={(event)=>event.stopPropagation()}>
        ...
        <div className="profile-v2-badge-preview"><BiblicalBadgeAvatar avatarId={draft.avatarId} profilePhotoUrl={draft.profilePhotoUrl||""} badgeId={previewBadge.id} size={230} locked={!summary.calculated.has(previewBadge.id)} dimWhenLocked={false} title={previewBadge.name}/></div>
