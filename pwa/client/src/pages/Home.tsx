...
function AdminView({ session, onLogin }: { session: PwaSession | null; onLogin: () => void }) {
  ...
  const videos = useLiveCollection("conteudos_videos", []);
  const audios = useLiveCollection("conteudos_audios", []);
  const books = useLiveCollection("conteudos_books", []);
  const courses = useLiveCollection("ibr_courses", []);
  const [showCreate, setShowCreate] = useState(false);
  const [showRequests, setShowRequests] = useState(false);
  const pendingMembers = pending.filter((item) => item.isApproved !== true);
  const totalContent = videos.length + audios.length + books.length;
  const approveMember = async (member: CollectionItem) => { try { await approveMemberRequest(member.id); toast.success(`${member.name || "Membro"} aprovado e sincronizado.`); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível aprovar agora."); } };
  const cards = [["Solicitações", String(pendingMembers.length), Users], ["Mídia ativa", String(totalContent), ImageIcon], ["Alunos IBR", String(pending.filter((item) => item.isIbr === true).length), School], ["Cursos IBR", String(courses.length), LayoutDashboard]] as const;
  return <section className="page-pad admin-view"><PageIntro eyebrow="ÁREA ADM" title="Tudo em ordem para servir melhor" text="Os dados são sincronizados em tempo real com o aplicativo Android." />
    <div className="admin-stats">{cards.map(([label, value, Icon]) => <button key={label} onClick={() => label === "Solicitações" ? setShowRequests((open) => !open) : toast.message(`${label}: ${value} registro(s) sincronizado(s).`)}><Icon size={19}/><strong>{value}</strong><span>{label}</span></button>)}</div>
    <section className="admin-work"><p className="eyebrow">GESTÃO RÁPIDA</p><h2>O que você precisa fazer agora?</h2><button onClick={() => setShowRequests((open) => !open)}><Users size={20}/><span>{pendingMembers.length ? `Revisar ${pendingMembers.length} solicitação(ões)` : "Solicitações em dia"}</span><ChevronRight size={18}/></button><button onClick={() => setShowCreate(true)}><Plus size={20}/><span>Adicionar vídeo, áudio ou livro</span><ChevronRight size={18}/></button><button onClick={() => toast.message(`${courses.length} curso(s) IBR e ${courses.reduce((total, course) => total + (course.chapters?.length || 0), 0)} aula(s) são administrados pelo Android.`)}><Settings2 size={20}/><span>Ver indicadores do IBR</span><ChevronRight size={18}/></button></section>
    {showRequests && <section className="admin-request-list"><div><p className="eyebrow">SOLICITAÇÕES DE ACESSO</p><h2>{pendingMembers.length ? "Aguardando aprovação" : "Nenhuma solicitação pendente"}</h2></div>{pendingMembers.map((member) => <article key={member.id}><span>{(member.name || "M").slice(0, 1).toUpperCase()}</span><div><strong>{member.name || "Membro"}</strong><small>{member.phone || "Telefone não informado"}{member.isIbr ? " · IBR" : ""}</small></div><button onClick={() => void approveMember(member)}>Aprovar</button></article>)}</section>}
    <section className="admin-media-summary"><p className="eyebrow">MÍDIA SINCRONIZADA</p><h2>{videos.length} vídeos · {audios.length} áudios · {books.length} livros</h2><p>Use “Adicionar vídeo, áudio ou livro” e informe o link do conteúdo para que ele abra corretamente na PWA e no Android.</p></section>{showCreate && <AdminContentDialog session={session} onClose={() => setShowCreate(false)} />}
  </section>;
}

function AdminContentDialog({ session, onClose }: { session: PwaSession; onClose: () => void }) {
  const [type, setType] = useState<"video" | "audio" | "book">("video"); const [title, setTitle] = useState(""); const [description, setDescription] = useState(""); const [mediaUrl, setMediaUrl] = useState(""); const [coverUrl, setCoverUrl] = useState(""); const [credit, setCredit] = useState(""); const [busy, setBusy] = useState(false);
  const typeLabel = type === "video" ? "vídeo" : type === "audio" ? "áudio" : "livro";
  const publish = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); try { await createAdminContent({ type, title, description, mediaUrl, coverUrl, credit }); try { const result = await sendPwaPush({ title: type === "video" ? `Foi adicionado o vídeo ${title.trim()}` : type === "audio" ? `Foi adicionado ${title.trim()} em mídia` : `Foi adicionado o livro ${title.trim()}`, body: description.trim() || "Abra o MIC Rhema para acompanhar.", link: "https://bichocutela.github.io/Mic-RHEMA-Ia-Studio-Google/" }); toast.success(`${typeLabel[0].toUpperCase()}${typeLabel.slice(1)} publicado e aviso enviado para ${result.sent} PWA(s).`); } catch (notificationError) { toast.message(notificationError instanceof Error ? notificationError.message : "Conteúdo publicado; o aviso poderá ser enviado depois."); } onClose(); } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível publicar."); } finally { setBusy(false); } };
  return <div className="content-dialog-backdrop"><form className="auth-dialog" onSubmit={publish}><button className="dialog-close" type="button" onClick={onClose}><X size={19}/></button><p className="eyebrow">NOVO CONTEÚDO</p><h2>Publicar em mídia</h2><p>O conteúdo aparece na PWA e no Android pelo Firestore, com link de abertura compatível.</p><label>Tipo<select value={type} onChange={(event) => setType(event.target.value as "video" | "audio" | "book")}><option value="video">Vídeo</option><option value="audio">Áudio</option><option value="book">Livro / PDF</option></select></label><label>Título<input value={title} onChange={(event) => setTitle(event.target.value)} required placeholder={`Nome do ${typeLabel}`}/></label><label>Descrição<input value={description} onChange={(event) => setDescription(event.target.value)} required placeholder="Breve descrição"/></label><label>{type === "video" ? "URL do vídeo" : type === "audio" ? "URL do áudio" : "URL do livro/PDF"}<input value={mediaUrl} onChange={(event) => setMediaUrl(event.target.value)} required placeholder="https://..."/></label><label>URL da capa (opcional)<input value={coverUrl} onChange={(event) => setCoverUrl(event.target.value)} placeholder="https://..."/></label><label>{type === "book" ? "Autor" : type === "audio" ? "Artista/ministrante" : "Ministrante (opcional)"}<input value={credit} onChange={(event) => setCredit(event.target.value)} placeholder="Nome"/></label><button className="solid-button" disabled={busy}>{busy ? "Publicando…" : `Publicar ${typeLabel}`}<ArrowRight size={17}/></button></form></div>;
}
...
function CommunityView({ view, session, onLogin }: { view: "prayer" | "members" | "team" | "donations" | "about"; session: PwaSession | null; onLogin: () => void }) {
  ...
  }[view as Exclude<typeof view, "prayer">];
...
function SignInDialog({ onClose, onSuccess, initialAdmin = false }: { onClose: () => void; onSuccess: (session: PwaSession) => void; initialAdmin?: boolean }) {
  const [name, setName] = useState(""); const [phone, setPhone] = useState(""); const [password, setPassword] = useState(""); const [admin, setAdmin] = useState(initialAdmin); const [requesting, setRequesting] = useState(false); const [busy, setBusy] = useState(false);
...
export default function Home() {
  ...
  >{viewComponent}{showLogin && <SignInDialog onClose={() => setShowLogin(false)} onSuccess={persistSession} />}{showAdminLogin && <SignInDialog initialAdmin onClose={() => setShowAdminLogin(false)} onSuccess={persistSession} />}</PwaShell>;
}
